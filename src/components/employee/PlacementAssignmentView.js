    ...
    <div className="bg-[#f8fafc] rounded-2xl shadow-sm border border-blue-100 p-6 sm:p-8 w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        <div>
          ...
          <p className="text-xl font-extrabold text-[#092a49] leading-tight break-words">
      ...
      {/* NAYA BLOCK: ACTUAL SALARY DISPLAY (Sirf tab dikhega jab data hoga) */}
      {employee.actualSalary && (
        <div className="mt-8 flex flex-col sm:flex-row gap-4 items-start sm:items-center">
          <div className="p-4 bg-teal-50 border border-teal-200 rounded-xl inline-flex items-center gap-4 shadow-sm w-full sm:w-auto">
            <div className="w-10 h-10 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 shrink-0">
              <span className="font-extrabold text-xl">₹</span>
            </div>
            <div>
              <p className="text-xs font-bold text-teal-800 tracking-wider uppercase mb-0.5">
                Final Joining Salary
              </p>
              <p className="text-2xl font-extrabold text-teal-900 leading-tight">
                ₹{employee.actualSalary}
              </p>
            </div>
          </div>
          <p className="text-xs font-semibold text-gray-500 max-w-xs">
            * This final amount will be used to calculate commercial payouts.
          </p>
        </div>
      )}
