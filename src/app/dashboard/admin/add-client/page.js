...
export default function AddClientPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    companyType: 'BPO', natureOfBusiness: '', description: '', status: 'Active',
    ...
    payoutDetails: { 
      ...
      payoutDuration: '', replacementTime: '', paymentTerms: '' 
    },
  ...
  const removeSlab = (catIndex, slabIndex) => {
    const updated = { ...formData.payoutDetails };
    updated.commercials[catIndex].slabs = updated.commercials[catIndex].slabs.filter((_, i) => i !== slabIndex);
    setFormData({ ...formData, payoutDetails: updated });
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-4 border-b pb-2">
            <h2 className="text-lg font-bold text-[#092a49]">2. Payout Structure (Slabs)</h2>
            <button type="button" onClick={addCommercialCategory} className="text-sm bg-[#092a49] text-white px-3 py-1 rounded-md font-bold hover:bg-[#183e61] transition-colors">+ Add Category</button>
          </div>
          
          <div className="space-y-6 mb-8">
            {formData.payoutDetails.commercials.map((categoryItem, catIdx) => (
              <div key={catIdx} className="bg-blue-50/40 border border-blue-100 rounded-xl p-4 relative">
                
                {/* Remove Category Button */}
                {formData.payoutDetails.commercials.length > 1 && (
                  <button type="button" onClick={() => removeCommercialCategory(catIdx)} className="absolute top-4 right-4 text-red-500 hover:text-red-700 p-1.5 bg-white rounded-md shadow-sm border border-red-100">
                ...
                <div className="mb-4 pr-10">
                  <label className="block text-xs font-bold text-blue-800 uppercase tracking-wider mb-1">Process / Category Name</label>
                  <input type="text" value={categoryItem.category} onChange={(e) => handleCategoryChange(e, catIdx)} className="w-full sm:w-2/3 p-2 border border-blue-200 rounded-md focus:ring-2 focus:ring-blue-500 outline-none text-sm font-semibold text-[#092a49]" placeholder="e.g. For CSR level the payout will be:" />
                </div>
                ...
                <div className="bg-white p-3 rounded-lg border border-gray-200">
                  <div className="flex justify-between items-center mb-2 border-b pb-2 border-gray-100">
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider">Slabs & Rates</label>
                    <button type="button" onClick={() => addSlab(catIdx)} className="text-xs text-[#0796fe] hover:text-[#0680d9] font-bold flex items-center gap-1">+ Add Slab</button>
                  </div>
                  ...
                  <div className="space-y-2">
                    {categoryItem.slabs.map((slab, slabIdx) => (
                      <div key={slabIdx} className="flex gap-2 items-center">
                        ...
                        <input type="text" value={slab.slabDetails} onChange={(e) => handleSlabChange(e, catIdx, slabIdx)} className="flex-1 p-2 border border-gray-200 rounded-md focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder="e.g. Rs. 2000/- (0-03 per candidate)" />
                        
                        {categoryItem.slabs.length > 1 && (
                          <button type="button" onClick={() => removeSlab(catIdx, slabIdx)} className="text-gray-400 hover:text-red-500 p-1.5 shrink-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-gray-100 pt-6">
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Pay-out Duration</label><input type="text" name="payoutDuration" value={formData.payoutDetails.payoutDuration} onChange={(e) => handleChange(e, 'payout-basic')} className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. 30 Days" /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Replacement Time</label><input type="text" name="replacementTime" value={formData.payoutDetails.replacementTime} onChange={(e) => handleChange(e, 'payout-basic')} className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. 60 Days" /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Payment Terms</label><input type="text" name="paymentTerms" value={formData.payoutDetails.paymentTerms} onChange={(e) => handleChange(e, 'payout-basic')} className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. Post joining & invoice submission" /></div>
          </div>
        </div>
